...
(function(compId){
   ...
   var htFallbacks;
var testEle=document.createElement("div");function isSupported(a){var d=testEle.style,f;for(i=0;i<a.length;i++)if(f=a[i],d[f]!==void 0)return!0;return!1}function supportsRGBA(){testEle.cssText="background-color:rgba(150,255,150,.5)";if((""+testEle.style.backgroundColor).indexOf("rgba")==0)return!0;return!1}
...
AdobeEdge.requestResources=AdobeEdge.requestResources||function(a,d){AdobeEdge.yepnope.errorTimeout=4E3;AdobeEdge.preload.busy=!0;AdobeEdge.preload.got=AdobeEdge.preload.got||{};var f,b=a.length,h=[],c;for(f=0;f<b;f++){c=a[f];if(typeof c==="string")url=c,c={load:url};else if(url=c.yep||c.load,c.callback){var k=c.callback;c.callback=function(a,b,c){k(a,b,c)&&d(a,b,c)}}if(!c.callback)c.callback=d;if(!AdobeEdge.preload.got[url])h.push(c),AdobeEdge.preload.last=url}h.length&&AdobeEdge.yepnope(h)};
...
function splitUnits(a){var d={};d.num=parseFloat(a);if(typeof a=="string")d.units=a.match(/[a-zA-Z%]+$/);if(d.units&&typeof d.units=="object")d.units=d.units[0];return d}function defaultUnits(a){var d=a,a=splitUnits(a);if(!a||!a.units)d+="px";return d}function findNWC(a,d){if(a.className==d)return a;for(var f=a.childNodes,b=0;b<f.length;b++){var h=findNWC(f[b],d);if(h!=!1)return h}return!1}
function simpleContent(a,d,f){var b=document.getElementsByTagName("body")[0],f=f||findNWC(b,compId),h,c,k,g;if(f){if(f.style.position!="absolute"&&f.style.position!="relative")f.style.position="relative"}else f=b;for(var m=0;m<a.length;m++){b=a[m];b.type=="image"?(h=document.createElement("img"),h.src=b.fill[1]):h=document.createElement("div");h.id=b.id;g=h.style;if(b.type=="text"){if(c=b.font){if(c[0]&&c[0]!=="")g.fontFamily=c[0];typeof c[1]!="object"&&(c[1]=[c[1]]);c[1][1]||(c[1][1]="px");if(c[1][0]&&
c[1][0]!=="")g.fontSize=c[1][0]+c[1][1];if(c[2]&&c[2]!=="")g.color=safeColor(c[2]);if(c[3]&&c[3]!=="")g.fontWeight=c[3];if(c[4]&&c[4]!=="")g.textDecoration=b.font[4];if(c[5]&&c[5]!=="")g.fontStyle=b.font[5]}if(b.align&&b.align!="auto")g.textAlign=b.align;if(b.position)g.position=b.position;if((!b.rect[2]||b.rect[2]<=0)&&(!b.rect[3]||b.rect[3]<=0))g.whiteSpace="nowrap";h.appendChild(document.createTextNode(b.text))}if(d)h.className=d;g.position="absolute";c=b.rect[0];k=b.rect[1];if(b.transform&&b.transform[0]){var j=
b.transform[0][0],e=splitUnits(j);if(e&&e.units&&(j=e.num,e.units=="%"&&b.rect[2])){var e=b.rect[2],l=splitUnits(b.rect[2]);if(l&&l.units)e=l.num,l.units=="%"&&(e=e/100*f.offsetWidth);j=j/100*e;f.offsetWidth>0&&(j=j/f.offsetWidth*100)}if((e=splitUnits(c))&&e.units)c=e.num;c+=j;if(!e.units)e.units="px";c+=e.units;if(b.transform[0].length>1){j=b.transform[0][1];if((e=splitUnits(j))&&e.units)if(j=e.num,e.units=="%"&&b.rect[3]){e=b.rect[3];if((l=splitUnits(b.rect[3]))&&l.units)e=l.num,l.units=="%"&&(e=
e/100*f.offsetHeight);j=j/100*e;f.offsetHeight>0&&(j=j/f.offsetHeight*100)}if((e=splitUnits(k))&&e.units)k=e.num;k+=j;if(!e.units)e.units="px";k+=e.units}}g.left=defaultUnits(c);g.top=defaultUnits(k);g.width=defaultUnits(b.rect[2]);g.height=defaultUnits(b.rect[3]);if(b.linkURL)htLookup[h.id]=b,h.onclick=function(){var a=htLookup[this.id];a.linkTarget?window.open(a.linkURL,a.linkTarget):window.location.href=a.linkURL},g.cursor="pointer";f.appendChild(h);if(b.c)for(g=0;g<b.c.length;g++)simpleContent(b.c[g],
d,h)}}var fnCycle=function(a){a?fnCycle&&setTimeout(fnCycle,20):a={event:"loading",progress:0};loadingEvt&&loadingEvt(a)},aBootcompsLoaded=[];if(!window.AdobeEdge.bootstrapListeners)window.AdobeEdge.bootstrapListeners=[];window.AdobeEdge.bootstrapCallback=function(a){window.AdobeEdge.bootstrapListeners.push(a);if(aBootcompsLoaded.length>0)for(var d=0;d<aBootcompsLoaded.length;d++)a(aBootcompsLoaded[d])};if(!window.AdobeEdge.preloadComplete)window.AdobeEdge.preloadComplete={};
window.AdobeEdge.preloadComplete[compId]=function(a){jQuery(".edgePreload"+a).css("display","none");fnCycle=null;loadingEvt&&loadingEvt({event:"done",progress:1,reason:"complete"});aBootcompsLoaded.push(a);for(var d=window.AdobeEdge.bootstrapListeners.length,f=0;f<d;f++)try{window.AdobeEdge.bootstrapListeners[f](a)}catch(b){console.log("bootstrap error "+b)}};function isCapable(){if(hasTransform){if(requiresSVG&&!hasSVG)return!1;return!0}return!1}
   ...
   aLoader = [
    ...
    { load: "edge_includes/edge.0.1.6.min.js"},
        {test: !hasJSON, yep:"edge_includes/json2_min.js"},
          { load: "Showcase_edge.js"},
          { load: "Showcase_edgeActions.js"}];
